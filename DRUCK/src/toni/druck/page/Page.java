...
public class Page extends EventAction {
    ...
    private void printWhenFreePlace(DataItem d, PageRenderer out,
        ...
        boolean platzDa = hasPlaceFor(controller, d); // entscheiden, ob genug
                                                      // Platz auf der Seite ist
        if (!platzDa) {
            if (v != null)
                v.restoreValues(aktualValues); // alte Werte kurz
                                               // zurückschreiben
            printNewPage(out);
            if (v != null)
                v.fillValues(d); // nun wieder mit den neuen Werten füllen
        }
        if (v != null) {
            v.perform(d, out);
        }
